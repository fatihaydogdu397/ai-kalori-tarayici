...
> **Amaç:** Uygulamadaki tüm ekranları, alt akışları, backend endpoint'lerini ve bağlantı durumlarını tek dosyada görmek. **Son pull:** `eatiq_backend@main` (2026-04-26). Mobil: Flutter 3.41 / Dart 3.11.

---

## 🚀 Implementation Status — 2026-04-26 Pre-Launch Polish

**Jira:** Fatih uygulamayı dolaştı, 22 madde paylaştı. Backend audit ile 1 BE-blocked + 4 BE-only ayıklandıktan sonra 17 mobile task tek oturumda DONE'a alındı.

**Açılan epic'ler (3):**
- [EAT-149](https://baseflowtech.atlassian.net/browse/EAT-149) Pre-Launch Mobile Polish — 18 task
- [EAT-150](https://baseflowtech.atlassian.net/browse/EAT-150) Diet Plan Concrete Foods & Meal Tracking — 4 task (BE+Mobile pair)
- [EAT-151](https://baseflowtech.atlassian.net/browse/EAT-151) Auth & Token Stability — 1 BE task

**Mobil tarafı tamamlandı (DONE — 17 task, dart analyze 0 error):**

| Task | Konu | Etkilenen dosya(lar) |
|------|------|---------------------|
| ✅ [EAT-152](https://baseflowtech.atlassian.net/browse/EAT-152) | Native splash (cold-start blank gap fix) | `pubspec.yaml`, `ios/Runner/Base.lproj/LaunchScreen.storyboard`, `android/app/src/main/res/{values,values-night,values-v31,values-night-v31}/styles.xml`, `…/drawable*/launch_background.xml`, generated splash assets |
| ✅ [EAT-153](https://baseflowtech.atlassian.net/browse/EAT-153) | Drop "Name" step from onboarding | `onboarding_screen.dart` (page count 13→11; name from `AppProvider.userName`) |
| ✅ [EAT-154](https://baseflowtech.atlassian.net/browse/EAT-154) | Global tap-to-dismiss keyboard | **NEW** `lib/widgets/keyboard_dismisser.dart`, `main.dart` (MaterialApp.builder) |
| ✅ [EAT-156](https://baseflowtech.atlassian.net/browse/EAT-156) | Optional blood-test upload step in onboarding | `onboarding_screen.dart` (page 10 `_PageBloodTestOptional`, multi-upload via `BloodTestUploadScreen`) |
| ✅ [EAT-157](https://baseflowtech.atlassian.net/browse/EAT-157) | Remove "You're set" final summary | `onboarding_screen.dart` (`_PageSummary` removed) |
| ✅ [EAT-158](https://baseflowtech.atlassian.net/browse/EAT-158) | Privacy Policy & Terms in-app WebView | **NEW** `lib/widgets/webview_screen.dart`, **NEW** `lib/screens/privacy_policy_screen.dart` (`baseflow.ee/privacy/`), **NEW** `lib/screens/terms_of_service_screen.dart` (`baseflow.ee/terms/`), `settings_screen.dart` + `paywall_screen.dart` linklendi |
| ✅ [EAT-159](https://baseflowtech.atlassian.net/browse/EAT-159) | Restore Purchases — 3-branch UX | `purchase_service.dart` (yeni `RestoreResult` enum: success / noEntitlement / error), `app_provider.dart`, `paywall_screen.dart` |
| ✅ [EAT-160](https://baseflowtech.atlassian.net/browse/EAT-160) | Delete Account in Settings | UI mevcuttu (`_confirmDeleteAccount` settings_screen.dart:545); audit doğrulandı |
| ✅ [EAT-161](https://baseflowtech.atlassian.net/browse/EAT-161) | Logout button in Settings ACCOUNT section | `settings_screen.dart` (`_confirmLogout` + ACCOUNT Column'a Logout row) |
| ✅ [EAT-162](https://baseflowtech.atlassian.net/browse/EAT-162) | Hide "Add to favorites" when already favorited | `history_screen.dart`, `home_screen.dart` (conditional `if (!analysis.isFavorite)`) |
| ✅ [EAT-163](https://baseflowtech.atlassian.net/browse/EAT-163) | Min 3 chars before food-search request | `food_search_screen.dart` `_onSearchChanged`, `dietary_anamnesis_screen.dart` `_search` (DislikedFoods page) |
| ✅ [EAT-164](https://baseflowtech.atlassian.net/browse/EAT-164) | App restart routing bug (4 case matrix) | `splash_screen.dart` `_checkOnboarding` rewrite, `app_provider.dart` `setOnboardingDone` setter, BE-source-of-truth (height>0 && weight>0 → flag set) |
| ✅ [EAT-165](https://baseflowtech.atlassian.net/browse/EAT-165) | Remove "Cuisine preferences" step | `dietary_anamnesis_screen.dart`, `diet_profile_edit_screen.dart` |
| ✅ [EAT-166](https://baseflowtech.atlassian.net/browse/EAT-166) | Remove "Grocery budget" step | `dietary_anamnesis_screen.dart` (`_PageCookingTime` budget UI), `diet_profile_edit_screen.dart` |
| ✅ [EAT-167](https://baseflowtech.atlassian.net/browse/EAT-167) | Swipe-back bug in diet onboarding | `dietary_anamnesis_screen.dart` (`PopScope` intercept; `_page>0` → `_back()`) |
| ✅ [EAT-168](https://baseflowtech.atlassian.net/browse/EAT-168) | Remove "Anything else?" notes step | `dietary_anamnesis_screen.dart`, `diet_profile_edit_screen.dart` |
| ✅ [EAT-169](https://baseflowtech.atlassian.net/browse/EAT-169) | Open uploaded blood-test PDFs in-app | **NEW** `lib/screens/pdf_viewer_screen.dart` (PdfViewer.uri + InteractiveViewer image fallback), `blood_tests_screen.dart` `_openFile()` |

**Yapı değişikliği:**
- Onboarding flow: **13 → 11 sayfa** (Name + Summary çıktı, BloodTest girdi → toplam 12 ile 1 net azaltım)
- Diet anamnesis: **6 → 4 sayfa** (Cuisines + Notes çıktı, CookingTime'dan budget kaldırıldı)
- Diet profile edit: Cuisine + Budget + Notes section'ları kaldırıldı
- Splash: 1400ms → 600ms transition; "Get Started" landing UI kaldırıldı (logged-out user direkt AuthScreen'e push); native splash arkasında Flutter splash sade logo + scanner visual kaldı
- Mevcut `splash_screen.dart` silindi → `lib/screens/getting_started_screen.dart` (welcome landing) ve sade native splash bridge ayrıldı

**Yeni dependency'ler (pubspec):**
- `flutter_native_splash: ^2.4.4` — iOS storyboard + Android styles autogen
- `webview_flutter: ^4.7.0` — Privacy/Terms in-app WebView
- `pdfrx: ^1.0.0` — Blood test PDF viewer

**BE-blocked (mobile beklemede):**
- 🟡 [EAT-155](https://baseflowtech.atlassian.net/browse/EAT-155) Diet type onboarding multi-select restore → BE [EAT-175](https://baseflowtech.atlassian.net/browse/EAT-175) (`dietType` scalar enum, multi-select için array kolonu gerekiyor)
- 🟡 [EAT-171](https://baseflowtech.atlassian.net/browse/EAT-171) Render concrete food items in diet-plan → BE [EAT-170](https://baseflowtech.atlassian.net/browse/EAT-170)
- 🟡 [EAT-173](https://baseflowtech.atlassian.net/browse/EAT-173) "I ate this meal" toggle UI → BE [EAT-172](https://baseflowtech.atlassian.net/browse/EAT-172) (`completeMeal` var, `uncompleteMeal` + auto-revert eksik)
- 🟡 [EAT-174](https://baseflowtech.atlassian.net/browse/EAT-174) BE auth investigation (spurious UNAUTHENTICATED, mobile follow-up gerekirse)

**Geçici çözümler / follow-up:**
- ⏳ Yeni eklenen string'ler (`EAT-156, 158, 161, 169`) inline TR/EN `_t(ctx, tr, en)` helper kullanıyor; 8 dilin (FR/ES/DE/AR/JA/ZH/PT/KA) ARB migrate'i pre-launch son aşamada toplu yapılacak
- ⏳ `saveAnamnesisProfile` BE şeması hâlâ `cuisines`/`budget`/`notes` required tutuyor; mobile boş değer geçiyor (`const []`, `'medium'`, `''`). BE temizliği [EAT-170](https://baseflowtech.atlassian.net/browse/EAT-170) tarafında yapılabilir.

**Commit:** `91f26b3` (origin/main) — 64 dosya, +2120/-1180
...
## ② Tam Navigation Akışı — 30 Ekran Tek Diyagramda

> Node renkleri: **mavi** = ana flow, **yeşil** = tab, **turuncu** = modal/sheet, **kırmızı** = eksik entegrasyon, **mor** = WebView/PDF viewer.

```mermaid
flowchart TD
    START([Uygulama Açılır]) --> NSP["🟦 Native Splash<br/>(LaunchScreen.storyboard /<br/>launch_background.xml)<br/>EAT-152"]
    NSP --> SP["🚀 Flutter Splash<br/>splash_screen.dart<br/>(600ms branded transition)"]

    SP -->|loggedIn=false| GS["👋 Getting Started<br/>getting_started_screen.dart"]
    SP -->|loggedIn=false (returning)| AUTH
    SP -->|loggedIn & onboarded & premium| HOME
    SP -->|loggedIn & onboarded & free| PAY
    SP -->|loggedIn & not-onboarded| ONB["📝 Onboarding<br/>11 step"]
    GS --> AUTH["🔐 Auth<br/>auth_screen.dart"]
    ...
    ONB -.optional step 10.-> ONB_BT["⬆️ Optional Blood-Test Upload<br/>(within onboarding)<br/>EAT-156"]
    ONB_BT --> BTU
    ONB --> PAY
    ...
    FAB --> SRC["🔍 Food Search<br/>food_search_screen.dart<br/>(min 3 chars guard EAT-163)"]
    ...
    T1 -->|see history| HIST["📅 History<br/>history_screen.dart<br/>(fav button hidden when favorited EAT-162)"]

    %% Program akışı
    T3 -->|plan yok| ANAM["📋 Dietary Anamnesis<br/>4 step form<br/>(swipe-back fixed EAT-167)"]
    ...
    SET --> PAY2["💎 Paywall"]
    SET --> PRV["📜 Privacy Policy<br/>privacy_policy_screen.dart<br/>(in-app WebView EAT-158)"]
    SET --> TOS["📜 Terms of Service<br/>terms_of_service_screen.dart<br/>(in-app WebView EAT-158)"]
    SET --> LO[["🚪 Logout<br/>(EAT-161, confirm dialog)"]]
    SET --> DA[["🗑️ Delete Account<br/>(EAT-160, danger dialog)"]]
    BT --> BTU["⬆️ Blood Test Upload<br/>blood_test_upload_screen.dart"]
    BT --> PDF["📄 PDF/Image Viewer<br/>pdf_viewer_screen.dart<br/>(in-app, EAT-169)"]
    ...
    classDef webview fill:#e7d6ff,stroke:#6f42c1,color:#000

    class NSP,SP,GS,AUTH,LOG,REG,FPO,RPW,ONB,ONB_BT,PAY,PAY2,HOME,RES,BAR,SRC,MAN,HIST,ANAM,DPL,WDP,SET,DPE,BT,BTU flow
    class T1,T2,T3,T4 tab
    class FAB,PPS,WSH,WGT,CAM,LO,DA modal
    class SF missing
    class AID,AIXX stub
    class PRV,TOS,PDF webview
```

**Okunuş:** Native splash artık cold-start blank gap'i kapatıyor (EAT-152) — Flutter splash 600ms branded transition. Splash routing 4 case'i intelligent yönetiyor (logged-in/out × onboarded/not, premium/free; EAT-164). Logged-out user direkt AuthScreen'e gidiyor (eski "Get Started" landing kalktı, kalan welcome içerikleri `getting_started_screen.dart`'a taşındı). Onboarding 11 step (Name + Summary + Cuisine + Budget + Anything-else çıktı, optional blood-test upload step girdi). Diet anamnesis 4 step (Cuisines + Notes + Budget kaldırıldı). Settings'ten Privacy/Terms in-app WebView'a, blood-test PDF'leri in-app PDF viewer'a açılıyor. AI Dietitian hâlâ **stub** (UnimplementedError).

---

## ③ Ekran Envanteri (30 ekran + 2 paylaşılan widget)

> Her satır: ekran adı — dosya:satır — özet — parent/children — kullandığı servis(ler). 2026-04-26 itibarıyla `social_login_screen.dart` legacy duplicate **silindi**; yerine 4 yeni ekran (GettingStarted, PrivacyPolicy, TermsOfService, PdfViewer) ve 2 paylaşılan widget (KeyboardDismisser, WebViewScreen) eklendi.

### A) Auth & Onboarding (8 ekran)

| # | Ekran | Dosya:Satır | Görev | Kullandığı Servis |
|---|-------|-------------|-------|-------------------|
| 1 | **Splash** | [lib/screens/splash_screen.dart](lib/screens/splash_screen.dart) | 600ms branded transition + routing (EAT-164: 4-case matrix). Native splash önce gösteriliyor, blank gap kapatılıyor (EAT-152) | `AppProvider.bootstrapSession`, `setOnboardingDone` |
| 1b | **Getting Started** | [lib/screens/getting_started_screen.dart](lib/screens/getting_started_screen.dart) | Welcome / "Get Started" landing — yalnız ilk-launch logged-out kullanıcı için splash sonrası görünür | — |
...
| 7 | **Onboarding** | [lib/screens/onboarding_screen.dart](lib/screens/onboarding_screen.dart) | **11 adım** profil formu: cinsiyet → doğum → boy/kilo → hedef → target weight → weekly pace → aktivite → diyet tipi → allerji/restriction → notification → optional blood-test upload (EAT-156) → tema. Name/Summary kaldırıldı (EAT-153/157). | `UserService.updateProfile`, `BloodTestService.uploadBloodTest` (optional) |
...
| 16 | **Dietary Anamnesis** | [lib/screens/dietary_anamnesis_screen.dart](lib/screens/dietary_anamnesis_screen.dart) | **4 adım**: restrictions → meals/day → cooking time → disliked foods. Cuisines + Notes + Budget kaldırıldı (EAT-165/166/168). Swipe-back artık önceki adıma gidiyor (EAT-167, `PopScope`). Min 3 chars search guard (EAT-163). | `UserService.updateProfile`, `NutritionService.foods` |
...
| 21 | **Diet Profile Edit** | [lib/screens/diet_profile_edit_screen.dart](lib/screens/diet_profile_edit_screen.dart) | Anamnez formunun düzenlenebilir versiyonu + regenerate. **Cuisine + Budget + Notes section'ları kaldırıldı** (EAT-165/166/168). Restrictions + Meals + Cooking Time kaldı. | `UserService.updateProfile`, `DietPlanService.generateDietPlan` |
| 22 | **Blood Tests** | [lib/screens/blood_tests_screen.dart](lib/screens/blood_tests_screen.dart) | Yüklenmiş kan testleri listesi. Tıklanırsa **in-app PdfViewer** (EAT-169) açılıyor (eski `url_launcher` external kalktı). | `BloodTestService.myBloodTests`, `deleteBloodTest` |
...
| 26 | **PDF/Image Viewer** | [lib/screens/pdf_viewer_screen.dart](lib/screens/pdf_viewer_screen.dart) | URL extension'a göre PDF (`PdfViewer.uri`) veya image (`InteractiveViewer + Image.network`) render. AppBar localized title, error state. (EAT-169) | `pdfrx` |
| 27 | **Privacy Policy** | [lib/screens/privacy_policy_screen.dart](lib/screens/privacy_policy_screen.dart) | `baseflow.ee/privacy/` URL'sini `WebViewScreen` ile in-app render. (EAT-158) | `webview_flutter` |
| 28 | **Terms of Service** | [lib/screens/terms_of_service_screen.dart](lib/screens/terms_of_service_screen.dart) | `baseflow.ee/terms/` URL'sini `WebViewScreen` ile in-app render. (EAT-158) | `webview_flutter` |

### E) Premium & AI (2 ekran)

| # | Ekran | Dosya:Satır | Görev | Kullandığı Servis |
|---|-------|-------------|-------|-------------------|
| 24 | **Paywall** | [lib/screens/paywall_screen.dart](lib/screens/paywall_screen.dart) | RevenueCat paketleri + purchase. **Restore Purchases 3-branch UX** (success/noEntitlement/error, EAT-159). Privacy/Terms linkleri in-app WebView'a. | `PurchaseService` (`RestoreResult` enum) + `AppProvider.restorePurchases/refreshPremiumStatus` |
...
### F) Paylaşılan Widget'lar (yeni — EAT-149 sprint çıktısı)

| # | Widget | Dosya | Görev |
|---|--------|-------|-------|
| W1 | **KeyboardDismisser** | [lib/widgets/keyboard_dismisser.dart](lib/widgets/keyboard_dismisser.dart) | Tap-to-dismiss keyboard wrapper. `MaterialApp.builder` üzerinden tüm Navigator stack'i kapsıyor. (EAT-154) |
| W2 | **WebViewScreen** | [lib/widgets/webview_screen.dart](lib/widgets/webview_screen.dart) | Generic in-app WebView base — URL + title alır, loading spinner + error retry. Privacy/Terms screen'leri buna sarılı. (EAT-158) |
| (mevcut) | **PortionPickerSheet** | [lib/widgets/portion_picker_sheet.dart](lib/widgets/portion_picker_sheet.dart) | Porsiyon + liquid + cooking method picker | — |
| (mevcut) | **LiquidWaveProgress** | [lib/widgets/liquid_wave_progress.dart](lib/widgets/liquid_wave_progress.dart) | Su göstergesi animasyonu | — |
...
|--------------|---------------------------------|-------|-----|
| **Splash** | `me` (bootstrap) | 🟢 | AppProvider.loadAuthStatus + refreshPremiumStatus + setOnboardingDone (EAT-164) |
| **Getting Started** | — | 🟢 | UI only, ilk-launch welcome landing |
...
| **Onboarding (11 step)** | `updateProfile`, optional `uploadBloodTest` | 🟢 | Step sonunda tüm profile push. Page 10 optional blood-test upload (EAT-156). Name/Summary kaldırıldı (EAT-153/157). |
...
| **Dietary Anamnesis (4 step)** | `updateProfile` (diet prefs) + optional `uploadBloodTest` | 🟢 | 4 adım form: restrictions → meals/day → cooking time → disliked foods. Cuisines/Notes/Budget kaldırıldı (EAT-165/166/168), BE'ye boş değer geçiyor. |
...
| **Settings** | `appSettings`, `updateAppSettings`, `updateProfile`, `deleteAccount`, `logout` | 🟢 | Notif + tema + hedef + dil + ACCOUNT section: Logout (EAT-161) + Delete Account (EAT-160). Privacy/Terms in-app WebView linkleri (EAT-158). |
| **Privacy Policy / Terms of Service** | — | 🟢 | In-app WebView (`webview_flutter ^4.7.0`); URL: `baseflow.ee/privacy/`, `baseflow.ee/terms/` (EAT-158) |
| **Diet Profile Edit** | `updateProfile`, `generateDietPlan` | 🟢 | Değişiklik → regenerate. Cuisine/Budget/Notes section'ları kaldırıldı. |
| **Blood Tests (list)** | `myBloodTests`, `deleteBloodTest` | 🟢 | List + long-press delete; row tap → in-app PDF/Image viewer (EAT-169) |
| **Blood Test Upload** | `uploadBloodTest` | 🟢 | Base64, max 10MB, JPG/PNG/PDF |
| **PDF/Image Viewer** | — | 🟢 | `pdfrx ^1.0.0` + `InteractiveViewer Image.network` (EAT-169) |
| **Paywall** | RevenueCat SDK → `POST /webhook/revenuecat` → `me.isPremium`; Restore: `Purchases.restorePurchases()` → `RestoreResult` enum | 🟢 | Client triggers, webhook validates. Restore 3-branch UX: success / noEntitlement / error (EAT-159) |
...
### 8.6 ✅ social_login_screen.dart duplicate — KALDIRILDI
- 2026-04-24 [EAT-137](https://baseflowtech.atlassian.net/browse/EAT-137) ile silindi.
...
### 8.10 🟡 Diet type multi-select — BE-blocked
- **Mobil:** [EAT-155](https://baseflowtech.atlassian.net/browse/EAT-155) ready, blocked. UI hâlâ single-select.
- **Backend:** [EAT-175](https://baseflowtech.atlassian.net/browse/EAT-175) açık. `dietType: DietType` scalar enum → `dietTypes: DietType[]` array kolonu lazım. AI prompt + recommendation service downstream güncellemesi gerekiyor.

### 8.11 🟡 Diet plan concrete foods + I-ate-this UI — BE-blocked
- **Mobil:** [EAT-171](https://baseflowtech.atlassian.net/browse/EAT-171) (food list render) + [EAT-173](https://baseflowtech.atlassian.net/browse/EAT-173) (I-ate-this toggle) blocked.
- **Backend:** [EAT-170](https://baseflowtech.atlassian.net/browse/EAT-170) (DietPlanMeal'a foods array'i ekle) + [EAT-172](https://baseflowtech.atlassian.net/browse/EAT-172) (`uncompleteMeal` mutation + Home/History deletion auto-revert linkage).

### 8.12 🟡 Auth & Token Stability — BE investigation
- **Backend:** [EAT-174](https://baseflowtech.atlassian.net/browse/EAT-174) — Spurious `UNAUTHENTICATED` errors on `dietPlanWeeklyLimit` / `myDietPlan` resolvers (200 OK + GraphQL errors[]). JWT validation race / refresh-token rotation hipotezleri.
- **Mobil:** Defensive token-refresh + retry-once gerekirse follow-up.

### 8.13 🟡 L10n — Inline TR/EN follow-up
- 2026-04-24 ve 2026-04-26 oturumlarında eklenen UI string'leri inline `_t(ctx, tr, en)` helper kullanıyor (`blood_test_upload_screen`, `dietary_anamnesis_screen` DislikedFoods page, `pdf_viewer_screen`, `webview_screen`, `settings_screen` Logout, Privacy/Terms title yer yer ARB). Pre-launch son aşamada 8 dil (FR/ES/DE/AR/JA/ZH/PT/KA) için ARB migrate'i toplu yapılacak.

### 8.14 🟡 saveAnamnesisProfile BE şema temizliği
- BE `cuisines`, `budget`, `notes` parametrelerini hâlâ required tutuyor; mobile UI bunları kaldırdı (EAT-165/166/168) ve boş değer geçiyor (`const []`, `'medium'`, `''`). BE temizliği [EAT-170](https://baseflowtech.atlassian.net/browse/EAT-170) sırasında veya ayrı task'la yapılabilir.

---

## ⑨ Pull Sonrası Yenilikler

### 2026-04-26 — Pre-Launch Polish (commit `91f26b3`)
17 mobile task tek committe:
- **Native splash** (flutter_native_splash) — iOS storyboard + Android styles + Void bg + app_icon_dark logo
- **Onboarding 11 step** — Name + Summary + Cuisine + Budget + Notes kaldırıldı, optional blood-test step eklendi
- **Diet anamnesis 4 step** — Cuisines + Notes + Budget kaldırıldı, swipe-back PopScope intercept
- **Splash routing 4-case matrix** — logged-in/out × onboarded/not, premium/free; `setOnboardingDone` setter; BE-source-of-truth (height>0 && weight>0 → flag set)
- **Settings ACCOUNT** — Logout button + Delete Account verified, Privacy/Terms in-app WebView linkleri
- **Restore Purchases** — `RestoreResult` enum, 3-branch UX (success/noEntitlement/error)
- **Min 3 chars search guard** — food_search + dietary_anamnesis disliked-foods
- **Hide favorites button** — favorited item'larda "Add to favorites" görünmez
- **Global keyboard dismisser** — MaterialApp.builder
- **In-app PDF/Image viewer** — pdfrx + InteractiveViewer
- **Dependency'ler:** flutter_native_splash ^2.4.4, webview_flutter ^4.7.0, pdfrx ^1.0.0

### 2026-04-23 — Backend Sprint (önceki)
...
- [lib/main.dart](lib/main.dart) — bootstrap, 10 dil, navigatorKey, KeyboardDismisser builder, fresh-install secure-storage purge
- [lib/services/app_provider.dart](lib/services/app_provider.dart) — ~1180+ satır merkezi state; `setOnboardingDone`, `restorePurchases→RestoreResult`
- [lib/services/api/api_client.dart](lib/services/api/api_client.dart) — GraphQL client, 401 retry, logout guard, dotenv runtime fallback
- [lib/services/api/token_storage.dart](lib/services/api/token_storage.dart) — secure storage wrapper
- [lib/services/purchase_service.dart](lib/services/purchase_service.dart) — RevenueCat + `RestoreResult` enum (success/noEntitlement/error)
- [lib/widgets/keyboard_dismisser.dart](lib/widgets/keyboard_dismisser.dart) — app-wide tap-to-dismiss
- [lib/widgets/webview_screen.dart](lib/widgets/webview_screen.dart) — generic WebView wrapper (Privacy/Terms)
- [lib/screens/pdf_viewer_screen.dart](lib/screens/pdf_viewer_screen.dart) — PDF/image agnostic viewer
- [pubspec.yaml](pubspec.yaml) — provider 6, http 1.2, flutter_secure_storage 9, purchases_flutter 8, health 12; **yeni:** flutter_native_splash 2.4.4, webview_flutter 4.7.0, pdfrx 1.0.0
...
| Profil + onboarding (11 step) | 🟢 Tam (Pre-Launch Polish 2026-04-26) |
| Diet anamnesis (4 step, swipe-back fixed) | 🟢 Tam |
| Native splash (cold-start gap) | 🟢 Tam (EAT-152) |
| Splash routing 4-case matrix | 🟢 Tam (EAT-164) |
| Kameradan AI analiz | 🟢 Tam |
| Barkod + manuel + arama (min 3 chars) | 🟢 Tam |
| History / Progress | 🟢 Tam |
| Water + Weight | 🟢 Tam |
| Diet Plan (anamnez + haftalık + limit) | 🟢 Tam |
| Diet Plan concrete foods + I-ate-this | 🟡 BE-blocked (EAT-170/172) |
| Diet type multi-select | 🟡 BE-blocked (EAT-175) |
| Blood Test upload + AI interpret | 🟢 Tam |
| Blood Test in-app PDF viewer | 🟢 Tam (EAT-169) |
| Privacy Policy + Terms (in-app WebView) | 🟢 Tam (EAT-158) |
| Settings: Logout + Delete Account | 🟢 Tam (EAT-160/161) |
| Premium / paywall (RevenueCat + webhook) | 🟢 Tam |
| Restore Purchases UX (3-branch) | 🟢 Tam (EAT-159) |
| Global tap-to-dismiss keyboard | 🟢 Tam (EAT-154) |
| Hide favorites button when favorited | 🟢 Tam (EAT-162) |
| Timezone-aware stats | 🟢 Tam |
| L10n — 10 dil ARB | 🟡 Yeni Pre-Launch string'leri inline TR/EN (8 dil son aşamada migrate) |
| Notification — local reminder | 🟡 Local |
| Notification — backend-triggered push | 🔴 Planned (EAT-144 blocked by EAT-115 IDEA) |
| AI Dietitian chat | ⚫ Stub / 🔴 Planned (EAT-143 blocked by EAT-114 IDEA) |
| Offline queue | 🟢 Local (EAT-141 mevcut) |
| Apple Health entegrasyonu | 🟡 Device-only, BE sync planlı (EAT-142) |
| Auth & Token Stability | 🟡 BE investigation (EAT-174) |

**En büyük gap'ler (sırayla):** (1) AI Dietitian chat — UI stub + BE endpoint yok, (2) Push notifications — BE servisi yok, (3) Apple Health → BE sync, (4) Diet plan multi-select & concrete foods (BE'de Kürşat'ta).

**Pre-launch ready:** 2026-04-26 itibarıyla mobile mainline temiz; pre-launch polish 17 task DONE. Geri kalan blocked task'lar BE bağımlı, mobile follow-up'lar BE bittikçe yapılacak.
